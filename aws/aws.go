...
func getRealState(session *session.Session) (resources resourceMap) {
	resources = reduce(
		...
		getIoT(session),
		getIoT1ClickDevicesService(session),
		getIoT1ClickProjects(session),
		getIoTAnalytics(session),
		getIoTEvents(session),
		getKinesis(session),
		...
		getSageMaker(session),
		getSchemas(session),
		getSdb(session),
		getSecretsManager(session),
		getServiceDiscovery(session),
		getSes(session),
		getSns(session),
		getSns(session),
		getSqs(session),
